...
public class BlackMovement : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        blackrb = GetComponent<Rigidbody2D>();
        blackAnimator = GetComponent<Animator>();
    }
    ...
    void Update()
    {
        if (!IsCentered)
        {
            ...
            if (transform.position.x < centerPosition.x && sideMovement == -1)
            {
                ...
                IsCentered = true;
                sideMovement = 0;
                blackAnimator.SetBool("Moving", false);
                blackAnimator.SetFloat("Horizontal", sideMovement);
            }
        ...
        if (inputEnabled)
        {
            ...
            if (sideMovement != 0)
            {
                ...
                blackAnimator.SetBool("Moving", true);
            }
            ...
            if (confirmInput == 1 && !FinalConfirm)
            {
                ...
                blackAnimator.SetTrigger("Confirm");
                FinalConfirm = true;
            }
        }
        ...
        {
            blackrb.MovePosition(blackrb.position + moveVelocity * Time.fixedDeltaTime);
        }
